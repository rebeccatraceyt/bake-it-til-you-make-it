...
**Find A Recipe Page** <br>
On the Find a Recipe Page a problem with the navbar became apparent. The overhanging navbar image was creating a problem whereby you could not click or select any elements that were 'under' it due to the z-index of the image. The z-index was necessary as not using it would issues with the placement of the scrolling feature of the navbar (where the navbar background changes slightly in order to remain visible) and so, an alternative solution needed to be found. The developer created a banner image that would act as a division between navbar and content in order to alleviate the issue in a more creative and less destructive way.
